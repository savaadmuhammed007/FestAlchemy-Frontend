...
export default function ScoringResults({
  ...
  const pendingCount = spinLottedPrograms.filter(p => !p.has_results).length;
  const inProgressCount = 0; // Standard default as shown in layout
  const submittedCount = spinLottedPrograms.filter(p => p.has_results && !p.is_published).length;
  const publishedCount = spinLottedPrograms.filter(p => p.is_published).length;

  // Extract unique categories for secondary filtering
  const uniqueCategories = Array.from(new Set(spinLottedPrograms.map(p => p.category_name))).filter(Boolean);

  // Filter programs based on tabs, dropdown, and search query
  const filteredPrograms = spinLottedPrograms.filter(p => {
    // 1. Search Query
  ...
  const handlePublishAll = async () => {
    const unpublished = spinLottedPrograms.filter(p => p.has_results && !p.is_published);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <div style={{ textAlign: 'center', flex: 1, minWidth: '200px' }}>
                <h3 style={{ color: 'var(--secondary-neon)', fontFamily: 'var(--font-display)', margin: 0 }}>
                  {spinLottedPrograms.find(p => p.id.toString() === resultsProgramId)?.name || 'Selected Program'}
                </h3>
                <span className="tag tag-primary" style={{ fontSize: '0.65rem', marginTop: '0.25rem', display: 'inline-block' }}>
                  {spinLottedPrograms.find(p => p.id.toString() === resultsProgramId)?.category_name || ''}
                </span>
              </div>
